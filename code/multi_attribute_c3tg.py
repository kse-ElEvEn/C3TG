...
def parse_args():
    ...
    parser.add_argument("--llama2_dir", type=str, required=True,
                        help="存放多个llama2_{dim}模型的路径")
    # BERT 多分类
...
def main():
    ...
    def load_dim_model(attr, lam):
        ...
        folder_name = mapping.get(attr, f"llama2_{attr}")
        path = f"{args.llama2_dir}/{folder_name}"
...
"""
        ...
        llama_response = askChatGPT(stage_prompt, model="gpt-4")
        if not llama_response:
            print(f"[Warn] GPT-4 stage{i} failed, using fallback => trivial prompt.")
            llama_response = f"Rewrite the text in a better way to reduce E.\n\n{txt_prev}"
